...
//
// ModelRouterUsecase.route() 는 primary 실패 시 fallback provider 를 "순차" 재시도한다
// (await primary.complete() → catch → await fallback.complete()). 따라서 한 번의 route()
// 호출은 최악의 경우 이 timeout 을 2회 누적한다 — codex full timeout(180s) → claude fallback(180s).
//
...
// route() 한 번의 worst-case latency — primary timeout + fallback timeout (순차 2회 누적).
