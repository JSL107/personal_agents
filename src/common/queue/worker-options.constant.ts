...
// 하므로 한 attempt 의 최악 LLM 시간은 단일 호출(300s)이 아니라 codex timeout 2회 누적
// = 600s (MODEL_ROUTER_WORST_CASE_MS) — 이다. 여기에 context fetch(GitHub/Notion/DB, 수십 초) +
// slack 발송(수 초)이 더해진다.
//
...
// → lockDuration 을 route() worst-case(600s) + context/slack 여유(90s) 를 흡수하도록 둔다.
