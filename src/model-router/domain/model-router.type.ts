...
export enum AgentType {
  ...
  SCHEDULE = 'SCHEDULE',
  // 라우터 자신 — 워커가 아니라 설비다. 사용자 요청을 받아 워커를 고르는 일만 하고,
  // **고르지 못했을 때만** 원장에 행을 남긴다(고른 회차는 그 워커의 행에 기록된다).
  //
  // 이 타입이 필요한 이유: 분류기가 UNKNOWN 을 내거나 dispatcher 없는 워커를 고르면
  // dispatch 이전에 예외가 나 AgentRun 이 아예 안 만들어졌다. 정확도 분석에 가장 필요한
  // "분류가 실패한 표본" 이 원장에 0건이었다는 뜻이다. 그 회차를 담을 자리가 이 값이다.
  //
