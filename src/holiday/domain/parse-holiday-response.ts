...
// 응답 전문 → 공휴일 목록. **형식이 어긋나면 빈 배열을 돌려준다** — 이 파서를 부르는 쪽
// (`KoreanHolidayApiClient`)이 HTTP 실패를 이미 예외로 끊으므로, 여기까지 온 것은 200 이다.
// 200 인데 형태가 다른 경우는 "그 달에 공휴일이 없음" 과 실무상 같은 결과로 두고, 어긋난
// 형태 자체는 호출부 로그가 건수 0 으로 드러낸다.
