...
export interface AutopilotTaskResult {
  ...
  //
  // ⚠️ `headlineText` 와 함께 주고 그 회차에 요약이 이것 하나뿐이면 자리가 뒤집힌다 —
  // 그림이 헤드라인과 함께 **메인 메시지**로 올라가고 `summaryText` 가 스레드로 내려간다
  // (orchestrator 참조). 여러 task 의 요약이 합쳐지는 회차에서는 그림이 어느 task 것인지
  // 드러나지 않으므로 종전대로 스레드에 붙는다.
  //
  ...
  //
  // 그림 없이 이것만 주면 아무 일도 하지 않는다 — 뒤집을 대상이 없으면 종전대로 나간다.
  headlineText?: string;
