...
// /ping (헬스체크), /sync-context (HOTFIX-1), /quota (OPS-1).
//
// C-4 Phase 2 — registerDiagnosisHandlers fn → @Injectable() class. PreviewActionHandler 패턴 답습.
@Injectable()
export class DiagnosisHandler implements SlackHandler {
  private readonly logger = new Logger(DiagnosisHandler.name);
  ...
  register(app: App): void {
    ...
    // (7c70a28 분할 이후 회귀 — 7f07136 의 원본 동작 복원)
    ...
    app.command('/sync-context', async ({ ack, command, respond }) => {
      ...
      await runEphemeral({
        ...
        task: () =>
          this.syncContextUsecase.execute({ slackUserId: command.user_id }),
    ...
    app.command('/quota', async ({ ack, command, respond }) => {
      ...
      await runEphemeral({
        ...
        commandLabel: '/quota',
        task: () =>
          this.getQuotaStatsUsecase.execute({
