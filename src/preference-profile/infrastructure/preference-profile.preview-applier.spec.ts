...
describe('PreferenceProfilePreviewApplier', () => {
  it('APPLIED → 반영 메시지 반환, applyService.apply 가 proposalId 로 호출됨', async () => {
    ...
    expect((applyService as { apply: jest.Mock }).apply).toHaveBeenCalledWith(
      42,
    );
  });
  ...
  it('STALE → STALE 포함 메시지 반환', async () => {
    const applyService = {
      apply: jest.fn().mockResolvedValue('STALE'),
    } as never;
    const applier = new PreferenceProfilePreviewApplier(applyService);
  ...
  it('NOT_FOUND → 찾을 수 없 포함 메시지 반환', async () => {
    const applyService = {
      apply: jest.fn().mockResolvedValue('NOT_FOUND'),
    } as never;
    const applier = new PreferenceProfilePreviewApplier(applyService);
  ...
  it('payload 에 numeric proposalId 없으면 throw', async () => {
    ...
    const applier = new PreferenceProfilePreviewApplier(applyService);
    await expect(applier.apply(buildPreview({}))).rejects.toThrow('proposalId');
    await expect(
      applier.apply(buildPreview({ proposalId: 'str' })),
