...
export class AgentRunService implements OnApplicationBootstrap {
  ...
  // 재기동 직후 한 번 쓸어낸다. 정리를 맡은 run-sweeper 는 매시 50분에만 돌고 임계가 30분이라,
  // 그 사이에 굳은 회차는 다음 정각 50분까지 원장에 "실행 중" 으로 남고 콘솔도 그 워커가 일하는
  // 중으로 그린다 — 재기동해도 최대 한 시간이다. 이 훅은 그 지연만 없앤다.
  //
  // 실측(2026-09-10 조회): `output->>'error' = 'swept: stale IN_PROGRESS'` 인 회차 20건 중
  // 16건이 정기 스윕이고 방치 시간은 31~59분이었다. 나머지 4건은 2026-07-26 23:51 에 한 번
  // 돈 초기 정리로, 5~28일 방치된 오래된 행을 함께 걷어낸 것이다.
  //
  // **왜 finish 가 안 남았는지는 이 훅이 고치지 못한다.** 원장에는 그 원인의 흔적이 없다 —
  // 스위퍼가 덮어쓴 `output` 에는 문구 한 줄뿐이고 `duration_ms` 는 20건 전부 null 이며,
  // `ended_at` 은 멈춘 시각이 아니라 청소한 시각이다(그 값으로 실제 지속시간을 추정하면 오독).
  // 서버 중단 구간이 유력한 후보지만(`worker-options.constant.ts` 주석이 같은 원인을 적어둔다)
  // 이 훅으로 검증되지는 않는다.
  //
