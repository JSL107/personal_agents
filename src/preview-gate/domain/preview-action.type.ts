...
export interface ApplyProgressState {
  ...
  //
  // **이 값은 시도가 끝나도 지우지 않는다.** 실패했든 마감했든 이미 반영된 단계는 이미 반영된
  // 것이고, 그 사실을 지우면 다음 승인이 그 단계를 처음부터 다시 실행한다 — 이 파일이 막으려는
  // 중복 반영이 실패 경로로 그대로 되돌아온다.
  done: string[];
  // 이 시도가 끝난 시각(ISO). **값이 있으면 지금 돌고 있지 않다.**
  //
  // `done`(어디까지 갔나)과 "지금 돌고 있나" 를 가르는 것이 이 필드다. 둘을 흔적의 존재 유무
  // 하나로 표현하던 동안은 끝날 때마다 흔적을 통째로 지워야 했고, 그래서 `done` 이 함께 사라졌다.
  // 없으면(undefined) 시도가 끝나지 않은 것 — 즉 그것을 쥔 프로세스가 죽었다는 뜻이고,
  // 부팅 훅은 그것만 집는다.
  endedAt?: string;
}
