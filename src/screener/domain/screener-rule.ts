...
//
// 4 로 올린 이유(2026-09-01): `high200Position` 의 기준이 최근 200봉의 **최고 종가**에서
// **장중 최고가**로 바뀌었다. 두 전략 모두 이 값을 순위 재료로 쓰므로(LONG_TERM 3순위,
// SWING 3순위) 같은 종목·같은 날이어도 순위가 달라진다. 장중에 찍고 내려온 고점이 이제
// 분모에 들어가 신고가 위치가 전반적으로 낮게 나오고, 위아래로 크게 흔들린 종목일수록
// 더 낮아진다. 앞 버전의 추천과 한 칸에 모으면 그 재편을 성적 변화로 읽게 된다.
