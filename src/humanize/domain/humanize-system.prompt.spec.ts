...
describe('번역투 처방 목록', () => {
  ...
  it('부정 안의 수식어는 건드리지 않게 한다 — 지우면 뜻이 뒤집힌다', () => {
    expect(HUMANIZE_SYSTEM_PROMPT).toContain(
      '부정·비교 안에 든 수식어는 그대로 둬라',
    );
  });

  // 번호 인덱싱에는 예외를 달았다가 뺐다. 예외가 있으면 모델이 유지 쪽으로 붙어 잔존이
  // 0 에서 3 으로 되돌아갔고, 걱정했던 참조 끊김은 실측에서 일어나지 않았다.
  it('번호 인덱싱에는 예외를 달지 않는다', () => {
