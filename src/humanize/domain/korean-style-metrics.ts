...
// 세 축을 켜도 걸리는 것이 없고, 기술 블로그 실측(2026-09-02 수집 · 토스 33.1 · 우아한형제들
// 28.7 · 하이퍼커넥트 37.7)을 기준으로 삼으면 **사용자 문체와 반대 방향으로 민다**.
//
// 편수를 적지 않는 이유 — 참조 코퍼스는 RSS 최신분이라 **수집 시점마다 편수와 값이 달라진다**
// (같은 스크립트로 09-02 오전 55편 · 오후 60편). 재현하려면 편수가 아니라 `scripts/
// fetch-reference-corpus.ts` 를 다시 돌려야 한다. 네이버 D2 는 위 범위에서 뺐다 — 그쪽 피드의
// 표·코드가 한 문장으로 붙어 편차가 228 로 잡힌 회차가 있었다(문장 축만 오염되고 구성 축은
// 멀쩡하다).
//
