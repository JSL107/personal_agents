...
[방금 만든 글의 실측]
평균 문장 길이가 ${measuredAverageLength}자로 나왔다. 문장이 토막토막 끊겨 숨이 가쁘게 읽힌다는 뜻이다.
**이어 붙여도 뜻이 상하지 않는 짧은 문장끼리 한 문장으로 합쳐라.**
...
**고치는 것은 숫자가 아니라 리듬이다.** ${BREATH_TARGET_MIN}자는 넘겼는지 보는 눈금일 뿐 목표가 아니다.
- 문단은 통째로 읽고 고쳐라. 앞쪽 몇 개만 합쳐 평균을 채우고 뒤에 토막 문장을 남기면, 긴 문장
  뒤에 조각이 붙어 고치기 전보다 어색하게 읽힌다. 문단 안에서 호흡이 고르게 흐르는지를 봐라.
- 합칠 자리가 없으면 합치지 마라. 억지로 이은 문장은 짧은 문장보다 나쁘다. 짧아야 할 문장은
  짧은 채로 두고, 끊어 갈 자리와 강조하는 자리의 짧은 문장은 그대로 살려라.`;
};
