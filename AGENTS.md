...
| 작업 | 먼저 볼 곳 |
|---|---|
| 슬래시 명령·재시도 | `src/slack/handler/agent-command.handler.ts` |
...
| 모델 선택·CLI 호출 | `src/model-router/application/model-router.usecase.ts` → `src/model-router/infrastructure/cli-process.util.ts` |
...
- HOME 은 `mkdtemp` 로 만든 throwaway 임시 디렉토리에 고정한다. Codex 인증은 실제 `CODEX_HOME` 을 전달해 보존한다. Claude의 `CLAUDE_CONFIG_DIR` 는 기본값을 주입하지 않고 부모가 명시했을 때만 전달한다.
