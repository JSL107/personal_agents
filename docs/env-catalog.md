...
SoT: `src/config/app.config.ts` 의 `EnvironmentVariables` (class-validator). 총 88개.
...
| `EVENING_RETRO_BLOG_NOTION_PAGE_ID` | ❌ | 블로그 발행 대상 Notion 부모 페이지 ID. 미설정 시 블로그 카드 승인은 명시 에러. |
| `PERSONAL_REPOS` | ❌ | CSV of owner/repo \| owner/* \| owner. 저녁 회고에서 개인 프로젝트로 라벨될 repo. |
