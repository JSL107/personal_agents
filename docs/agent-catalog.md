...
이대리의 에이전트 27종. SoT: `src/agent-registry/agent-registry.ts` + `AGENT_TO_PROVIDER`(`model-router.usecase.ts`).
...
| BE SRE | `BE_SRE` | — (webhook/자동) | CHATGPT | `src/agent/be-sre/application/analyze-stack-trace.usecase.ts` | 스택트레이스 분석 (수동 /be-sre) |
