...
> 작성 2026-08-18 · 상태: 결정 확정(§7), A안 구현 중
>
...
4. **승인 카드를 새로 만들지 않는다(결정).** 발행은 `published: false` — 비공개 초안으로만 들어가므로
   ...
   쪽(공개 게시)만 사람이 누른다.
5. 실행 지점 — 저녁 회고가 경력 프로필을 갱신하는 경로
   (`evening-retro-publish.autopilot-task.ts` / `EveningCareerReflectApplier`) 뒤에 사이트 발행을 잇는다.
   노션 append와 사이트 발행은 서로를 막지 않게 독립 실패 처리한다.
...
- 사이트의 커스텀 도메인·서브도메인 라우팅(사이트 레포 `docs/subdomain-routing-design.md` 소관).
