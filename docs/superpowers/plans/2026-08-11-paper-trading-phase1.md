...
케이스: ① KOSPI 매수는 수수료만·세금 0, ② KOSPI 매도는 수수료 + 거래세, ③ KONEX 매도 세율(0.10%)이 KOSPI·KOSDAQ(0.20%)과 다름, ④ 2025-12-29 체결분 전후로 세율이 갈린다(같은 시장·같은 금액이 날짜만 달라도 세금이 다름), ⑤ 계산 결과가 **원 단위 정수**(소수 원이 남지 않음), ⑥ 금액 0 이면 비용 0.

> KOSPI 와 KOSDAQ 은 총 세율이 같다(2025-12-29 이후 둘 다 0.20%). 세목 구성만 다르다 — KOSPI 는 거래세 0.05% + 농어촌특별세 0.15%, KOSDAQ 은 농특세 없이 0.20%. 총액이 같으므로 손익 계산에서는 구분되지 않는다. 시장별 차이가 실제로 드러나는 것은 KONEX(0.10%)다.
