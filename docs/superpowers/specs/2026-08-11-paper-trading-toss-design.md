...
### 실측으로 확인된 것 (2026-08-11)

실제 계좌를 개설해 굴려본 결과다.

- **토스 시세 조회는 `adjusted=false` 로 정상 동작한다.** 삼성전자 239,000원, KODEX 인버스 1,126원을 받았다.
- **비용·원가·실현손익 계산이 손 검산과 일치한다.** 삼성전자 4주를 239,500원에 매도했을 때 대금 958,000원 − 수수료 178원 − 증권거래세 1,916원(0.20%) = 955,906원이 현금에 들어오고, 실현손익은 675,854원(순수취 − 4주 × 평단 70,013)이다.
- **시세 캐시(`DailyPrice`)를 오염시키지 않는다.** 모의투자가 새로 만든 종목(삼성전자)의 `daily_price` 행은 시세를 조회한 뒤에도 0건이고, 같은 쿼리로 센 실계좌 동기화 종목(441640)은 4건이다 — 쿼리가 0만 내는 것이 아님을 대조군으로 확인했다.
- **같은 거래일 재실행은 스냅샷을 덮어쓴다.** 2회 실행 후에도 `paper_equity_snapshot` 1행, `paper_position_snapshot` 2행이다.

**장 마감 후에도 일봉 종가가 갱신된다.** 같은 날 19:05 에 239,500원이던 삼성전자 종가가 19:12 에 239,000원으로 바뀌었다. 시간외 단일가가 반영되는 것으로 보인다.

이것이 실행 시각 선택에 갖는 의미는 아직 확정하지 못했다. 17:40 값이 최종이 아닐 수 있다는 뜻이기도 하고, 반대로 **정규장 종가를 원한다면 시간외가 섞이기 전인 17:40 이 오히려 정확하고 19시 값이 오염된 것**일 수도 있다. 어느 쪽인지는 시간외 거래가 활발한 종목으로 17:40·18:30·19:30 세 시점을 비교해야 판정된다. overwrite 설계라 재실행하면 교정되므로 당장 위험은 아니지만, 성적의 기준 시점이 흔들리는 문제이므로 확인 대상으로 남긴다.
