...
수집 실행은 **로그로 남긴다.** `AgentRunService`로 감싸지 않는다.

이 레포에서 `AgentRunService`를 쓰는 것은 모델(LLM)을 부르는 에이전트뿐이다 — `agent`·`autopilot`·`router`·`humanize`·`pr-review-loop` 등. 반면 모델을 부르지 않는 수집·계산 계열(`screener`·`market-data`·`paper-trading`·`backtest`)은 하나도 쓰지 않고 `logger.log()`로 남긴다. 이 수집기는 후자에 속한다.

`AgentType`을 새로 만들면 등록부·계약·provider map·생성 문서로 번지고, 끝내 오피스 콘솔의 좌석 배치까지 건드리게 된다(실제로 성장방 정원이 꽉 차 소파가 자리를 잃었다). 모델을 부르지 않는 수집기에 사무실 자리를 만들어 줄 이유가 없다.

로그 한 줄에 남길 것: 소스별 상태, 3단 계수, 저장 건수.
부분 실패와 "수신은 있는데 검증 0"은 `CollectOutcome` 반환값으로 호출부에 전달돼 아침 카드에 노출된다 — 사용자 가시성은 그쪽에서 확보한다.
