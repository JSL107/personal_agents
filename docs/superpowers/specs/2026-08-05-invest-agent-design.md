...
| 2 | 실패가 보이나 | 강제 실패를 넣어 FAILED 가 기록되는지 | ✅ 아래 실측 |
...
**전체 실패 실측 (2026-08-05, 리뷰 반영 후)** — 보유 2종목이 모두 시세 조회에 실패하는
상황을 주입.

```
[예외] 보유 2종목을 한 건도 점검하지 못했습니다 — 005930.KS: … / 000660.KS: …
[행]   id=423  status=FAILED
[output] {"error":"보유 2종목을 한 건도 점검하지 못했습니다 — …"}
```

초기 구현은 종목별 실패를 `failures`에 모으기만 하고 항상 정상 반환했다. 그러면 보유 종목이
**전부** 실패한 날에도 원장에는 `SUCCEEDED`로 남는다 — 실패를 보이게 하려고 원장에 편입한
변경이 정작 실패를 감추는 자기모순이었다. 리뷰에서 지적되어 아래 계약을 넣었다.

| 조건 | 원장 | 근거 |
|---|---|---|
| `checkedCount > 0` (부분 실패) | SUCCEEDED | 한 종목이 막혀도 나머지를 처리하는 설계를 유지. 실패 목록은 요약에 실려 화면으로 나간다 |
| `checkedCount === 0 && failures > 0` | **FAILED** | 아무것도 못 했으면 실패다. `/retry-run` 대상이 되어야 한다 |

던진 예외는 `AgentRunService`가 FAILED로 기록하고 콘솔에 `FAILED` 상태를 발행한 뒤 다시
던지며, orchestrator가 잡아 그룹을 계속 돌리면서 owner 요약에 `⚠️ … 자동 생성 실패` 한 줄을
남긴다. **화면 알림을 잃지 않고 오히려 오피스 캐릭터가 실패 상태로 바뀐다.**
