...
| `CONTRADICTION_JUDGE` | 밴드 쌍 1건을 실제 judge 에 태워 **6.5초 만에 정상 판정 수신**(`contradiction=false` + 사유 문자열). L4 게이트는 미설정 = 활성 | **경로 정상.** 다만 발화 여부는 확인할 수 없다(§1.2) |
...
### 1.2 `CONTRADICTION_JUDGE` — 살아 있지만 발화를 확인할 방법이 없다

호출 경로가 정상임은 실증으로 확인했다. 그러나 **이 워커가 지난 일요일에 실제로 돌았는지는
어떤 기록으로도 판정할 수 없다.** 판정 결과가 어디에도 저장되지 않기 때문이다.
...
다만 소속 태스크인 `knowledge-lint` 전체의 발화 여부는 간접 확인이 가능하다. L1 중복 후보
쌍이 현재 **6,436건**이라 이슈 0건 skip 에 걸릴 수 없고, 따라서 **매주 일요일 10시에 중복
후보 보고가 Slack 으로 나가야 정상**이다. 그 보고의 수신 여부가 가장 직접적인 증거다.
...
- `CONTRADICTION_JUDGE` 는 `knowledge-lint` 의 일요일 10시 Slack 보고 수신 여부로 판정한다.
...
| 셸 기본 PATH | v20.20.2 | `nvm alias default` = `20` |
...
**하나, 셸 기본값이 20 이다.** nvm 기본 별칭이 `20` 이고 저장소에 `.nvmrc` 가 없어서, 새
터미널을 열면 선언과 다른 런타임을 집는다. 초판의 "이 머신의 런타임은 v20" 이라는 서술도 이
때문에 나왔다 — 셸에서 `node -v` 를 찍으면 20 이 나오지만 서비스는 22 로 돌고 있었다.
`.nvmrc` 에 `22` 한 줄을 두면 저장소에 들어올 때 자동으로 맞춰진다.

**둘, 타입 정의만 아직 Node 20 기준이다.** `engines` 는 `>=22` 인데 `@types/node` 는
