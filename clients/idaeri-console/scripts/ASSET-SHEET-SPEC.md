...
- **타일은 32×32 로 저장된다.** `TILE_PX` 가 규격이고, 화면 쪽 짝은
  `ConsoleCore/OfficeViewMetrics.swift` 의 `officeSpriteUnit` 이다. 두 값이 어긋나면 화면에서
  비정수 배율로 그려져 타일 격자선이 흐려진다.
- **가구·캐릭터는 규격을 강제하지 않는다.** 화면에서 실물 크기(`targetHeightCm`)를 기준으로
  스케일되므로 원본 크기를 바꾸면 그 계산의 입력이 흔들린다. 짧은 변을 32의 배수로 맞춰 봤더니
  `prop-papers` 가 10×6 에서 53×32 로 5배 부풀었다.
