...
## 규격과 팔레트 (2026-09-04 재정합)

- **타일은 40×40 으로 저장된다.** `TILE_PX` 가 규격이고, 화면 쪽 짝은
  `ConsoleCore/OfficeViewMetrics.swift` 의 `officeSpriteUnit`(= 40) 이다. 두 값이 어긋나면
  화면에서 비정수 배율로 그려져 타일 격자선이 흐려진다.
  40px 인 이유는 **원본 시트가 그 크기로 그려져 있기 때문**이다 — 1 도트가 8 화면픽셀이고
  타일 셀이 그 5배(320px)다. 검출 크기가 39~41px 로 흔들리던 것만 1px 이내로 스냅한다.
- **가구·캐릭터는 규격을 강제하지 않는다.** 화면에서 실물 크기(`targetHeightCm`)를 기준으로
  스케일되므로 원본 크기를 바꾸면 그 계산의 입력이 흔들린다. 짧은 변을 타일 규격의 배수로
  맞춰 봤더니 `prop-papers` 가 10×6 에서 53×32 로 5배 부풀었다(당시 시도한 32px 규격 기준).
  규격을 32px 로 내리는 안은 세라믹 타일의 1px 가로 이음매가 축소에서 버려져 격자선이
  사라지는 것을 확인하고 되돌렸다.
- **스프라이트 1개의 색 상한은 64색이고, 넘으면 빌드가 실패한다**(`PALETTE_MAX`).
  양자화는 `MEDIANCUT` + 디더링 없음 — 디더링을 켜면 점무늬가 생겨 도트가 다시 지저분해진다.
- **팔레트를 공유하는 것은 타일뿐이다**(`SHARED_PALETTE_MAX`). 타일은 같은 재질이 여러 파일에
  걸쳐 반복해 깔리므로 파일마다 색이 갈리면 얼룩이 된다. 가구 45종을 한 팔레트에 넣어 봤더니
  채도 높은 물건이 표를 독점해 회색이 5색만 받았고, 세라믹 타일의 격자선이 배경과 뭉쳐
  사라지면서 남은 픽셀이 엉뚱한 청록·베이지로 매핑됐다.
- **캐릭터는 팔레트 공유에서 빠진다.** 런타임 리컬러가 밝기·채도 임계값으로 머리·셔츠·바지를
  가르므로(`SpriteLoader.swift:43-47`) 색을 뭉치면 사람마다 옷 색이 뒤섞인다. 캐릭터를 교체하면
  아래로 대역이 유지되는지 확인할 것 — 파일별 양자화에서는 변화가 3~5% 였다.

```
바지 밝기 ≤ 15 · 머리 밝기 40~75 · 셔츠 밝기 ≥ 200 · 얼굴 채도 ≥ 26
```
