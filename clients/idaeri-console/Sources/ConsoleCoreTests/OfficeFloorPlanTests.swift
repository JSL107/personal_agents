...
func runOfficeFloorPlanTests(_ t: TestRunner) {
    ...
    //
    // 그래서 여기서는 **값이 임의로 되돌아가는 것만** 막는다. 실제 밝기 판정은 렌더 픽셀을
    // 직접 재는 `swift run IdaeriConsole --color-check` 가 한다 — 2026-09-08 실측으로
    // 복도 226.7 · 가장 밝은 방(세라믹) 212.3 · 벽 205 대이고, 복도가 화면에서 가장 밝다.
    // 벽과도 비교하지 않는다 — 벽은 이 값 위에 `wallBaseColor` 물들임을 따로 받아 같은 축의
    // 값이 아니다(그래서 벽은 아래 모델에서도 빠진다).
    t.expect(
    ...
    //
    // 텍스처 밝기는 `scripts/draw-tiles.py` 가 굽고 나서 출력하는 값을 **손으로 옮긴** 것이다.
    // 타일을 다시 구우면 이 표가 조용히 낡는데, 이 파일 안에서는 그것을 알 방법이 없다 —
    // 여기 적힌 숫자로 계산하고 여기 적힌 숫자로 판정하니 언제나 자기 자신과 맞는다.
    //
    // 그 대조는 `--color-check` 가 한다. 타일 PNG 에서 밝기를 다시 재고, 아래 모델
    // (`텍스처 × (1 - 누르기)`)이 렌더 픽셀을 아직 설명하는지(편차 12 이내) 확인한다.
    // 그래서 이 표가 낡거나 모델이 화면과 어긋나면 그쪽 게이트가 알려 준다.
    ...
    // 화면 쪽 같은 규칙은 `--color-check` 의 하한 190 이다(실측 최저 206.7).
