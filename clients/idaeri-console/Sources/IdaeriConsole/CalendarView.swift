...
struct CalendarView: View {
    ...
    private func createFailureReason(_ error: Error) -> String {
        ...
        switch status {
        case 400:
            // **원인을 하나로 단정하지 않는다.** 400 은 날짜만이 아니라 길이 상한에서도
            // 나는데, 응답 본문을 클라이언트가 버리므로(`sendExpectingSuccess`) 어느 쪽인지
            // 알 수 없다. 날짜만 짚으면 메모가 길어 막힌 사용자가 날짜만 고쳐 보다 만다.
            return "백엔드가 입력을 받지 못했습니다. 날짜와 제목·메모 길이를 확인하세요."
        case 404:
