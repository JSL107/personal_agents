...
///
/// 오피스 탭과 같은 픽셀 에셋을 재사용해 두 화면이 같은 회사처럼 보이게 한다.
///
/// 상태색이 닿는 자리는 둘뿐이다 — 좌상단 배지의 점, 그리고 책상 오른쪽 위 조명 얼룩
/// (흐린 원). 방의 벽·바닥·가구는 상태와 무관하게 따뜻한 목재·종이색으로 유지하고,
/// 카드 테두리(상태 강조색)는 이 뷰가 아니라 `AgentCardView` 가 그린다.
