...
final class CharacterNode: SKNode {
    ...
    private func currentPose() -> String {
        ...
        // **위로 걷는 동안은 한 장을 계속 쓴다.**
        //
        // 걸음은 두 그림을 번갈아 쓰는데(홀수=걸음, 짝수=정지), 그 정지 그림이 `idle`
        // 곧 **앞모습**이다. 앞으로 걸어올 때는 둘 다 앞모습이라 문제가 없지만, 위로 걸을
        // 때는 뒤통수와 얼굴이 한 걸음씩 번갈아 나타나 캐릭터가 앞뒤로 홱홱 돈다 —
        // 방향 그림을 붙이고도 "여전히 뒤로 걷는다" 로 보인 것이 이것이다.
        //
        // 정지 뒤모습 원화가 없으므로 교차를 포기하고 방향을 지킨다. 다리는 멈춘 것처럼
        // 보이지만 몸 기울기가 걸음을 나르고, 무엇보다 **얼굴이 섞이지 않는다.**
        // 정지 뒤모습이 들어오면 그때 짝수 걸음에 넣어 교차를 되살리면 된다.
        if facing == .up {
            return "walk-up"
        }
