...
final class CharacterNode: SKNode {
    ...
    private func currentPose() -> String {
        ...
        // **아래로 오는 걸음만 앞모습이다.** 나머지는 전부 뒤모습을 쓴다.
        //
        // 방향 판정(`facing(from:to:)`)은 타일 좌표로 갈리지만 화면은 3/4 시점이라,
        // **좌우 이동도 화면에서는 비스듬히 위로 올라간다.** 그 구간에 앞모습을 쓰면
        // 사람이 위로 올라가면서 얼굴을 보이는 그림이 되고, 그것이 "뒤로 걷는다" 로
        // 읽힌다 — 화면 녹화를 프레임으로 쪼개 확인한 결과, 계속 지적받던 장면이
        // `.up` 구간이 아니라 **좌우 구간**이었다.
        //
        // 옆모습 원화가 없으므로 앞뒤 중 하나를 골라야 한다. 화면에서 멀어지는 쪽으로
        // 움직이는 이상 뒤통수가 맞고, 무엇보다 위로 가는 구간과 섞이지 않아 방향이
        // 흔들리지 않는다. 짝수는 두 발을 모은 정지, 홀수는 한 발을 내디딘 걸음이다.
        if facing != .down {
