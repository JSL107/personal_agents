...
final class OfficeScene: SKScene {
    ...
    private func walk(
        ...
        // **상하 흔들림(bob)은 없앴다.**
        //
        // 도트 시절에는 이것과 좌우 기울임·착지 눌림이 걸음 그림을 대신하는 유일한
        // 신호였다. 걸음 원화가 들어온 뒤로 기울임·눌림은 역할이 겹쳐 걷어냈고 bob 만
        // "그림에 없는 성분" 이라는 이유로 남겨 뒀는데, 진폭을 0.11칸에서 0.035칸까지
        // 줄이고 그림이 있는 방향에만 얹어도 사용자에게는 계속 **통통 뛰는 것**으로
        // 읽혔다. 다리가 교차하는 그림 위에 상하 운동이 더해지면 걸음이 아니라 도약으로
        // 보이는 것이 이유다 — 실제 걸음에서 몸통은 거의 오르내리지 않는다.
        //
        // 그림이 없는 방향(한쪽 원화만 가진 캐릭터)에서는 이제 아무 몸짓 없이 미끄러진다.
        // 뒷걸음질로 튀는 것보다 낫고, 그 자리는 반대 방향 원화가 채워야 할 몫이다.
        for step in path {
