    ...
    private func walk(
        ...
        for step in path {
            ...
            let stepAction = SKAction.run { [weak self, weak node] in
                ...
                // **지금 방향의 걸음 그림이 있을 때만 얹는다.** bob 은 그림 위에 더해지는
                // 보조 신호인데, 그림이 없으면 그것이 **유일한 움직임**이 되어 다리가 멈춘
                // 채 몸만 위아래로 튄다 — 사용자가 "통통 뛰어다닌다" 고 본 것이 이것이다.
                //
                // 원화는 방향별로 한쪽씩만 있다 — 앞모습 일곱 명(1·2·3·16~19), 뒷모습
                // 열세 명(0·4~15). 그래서 이 검사는 **자주 거짓이 된다.** 앞모습만 가진
                // 사람이 위로 걸으면, 뒷모습만 가진 사람이 아래로 걸으면 그림이 안 바뀌므로
                // bob 을 빼고 미끄러지듯 지나간다. 제자리뛰기보다 덜 어색하다.
                //
                // 양쪽 원화가 다 채워지면 이 검사는 늘 참이 되지만 그때도 그대로 둔다 —
                // 원화가 빠지거나 새 인덱스가 늘어나는 순간 같은 증상이 조용히 돌아오는
                // 자리이고, 검사 비용은 캐시된 존재 판정 한 번이다.
