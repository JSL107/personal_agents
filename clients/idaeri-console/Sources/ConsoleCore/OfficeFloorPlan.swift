...
///
/// **그 진단은 도트 시절 것이다.** 위 계보(Gather·RPG Maker·스타듀밸리)는 전부 탑다운
/// 픽셀아트다. 이 화면은 #542 이후 3D 클레이 렌더 가구와 고해상도 원화를 쓰는데, 3/4
/// 시점에서 **사람은 정면 전신이 보이고 가구는 위에서 눌려 보인다.** 그래서 실물 비율을
/// 정확히 지킬수록 사람이 커 보인다 — 책상이 실물대로 사람 키의 66% 여도 화면에서는
/// 어깨가 상판보다 넓다(사용자 보고: "캐릭터가 가구 대비 크다").
///
/// 이 화면이 지켜야 할 것은 물리적 정확성이 아니라 **보이는 대로의 균형**이므로, 주석이
/// 남겨 둔 하한 0.85(→1.15칸)를 실제로 쓴다. 전체 렌더로 1.0·0.85·0.72 를 비교했다 —
/// 0.72 는 반대로 상판에 파묻혀 누가 앉았는지 알아보기 어렵다. 0.85 는 위 장르 범위
/// (1.1~1.4칸) 안에 그대로 있다.
public let officeCharacterScaleFactor: Double = 0.85

/// 스프라이트 픽셀 ↔ 실물 치수 환산 기준. 서 있는 캐릭터 54px 을 키 170cm 로 본다.
///
/// 3/4 시점이라 정밀한 값은 아니다. 쓸모는 절대 크기가 아니라 **가구끼리의 편차**를 재는 데 있다.
///
/// **캐릭터 배율과는 분리한다.** 한때 `officeCharacterScaleFactor` 를 함께 곱했다. 배율을
/// 내리면 "기준은 그대로인 채 캐릭터만 작아져 가구가 사람보다 커진다" 는 우려 때문이었는데,
/// 사람을 가구 대비 작게 만들려고 배율을 내리는 지금은 **그게 바로 노리는 것**이다. 환산까지
/// 따라 내려가면 가구도 같이 줄어 상대 비율이 그대로다 — 실제로 배율을 0.85 로 내렸을 때
/// 책상이 함께 줄어 책상 위 서류 더미가 상판을 넘었다(`OfficeFloorPlanTests` DeskPaperStack).
///
/// 그래서 이 기준은 **가구끼리의 편차를 재는 자**로만 남긴다. 배율 미적용 54px 이 170cm 이고,
/// 화면에 실제로 그려지는 사람 키는 여기에 배율을 곱한 값이다. 둘을 섞으면 자가 같이 늘었다
/// 줄었다 해서 무엇을 재는지 알 수 없어진다.
...
///
/// **값은 전체 화면 렌더로 잡는다 — 방 하나를 확대한 그림으로 재면 틀린다.**
///
/// 한 번 그렇게 틀렸다. 0.30 이 "책상에 걸터앉은" 것으로 보인다는 보고를 받고 `--room` 확대
/// 렌더로 0.30·0.55·0.80 을 비교해 0.52 로 올렸는데, **방향이 반대였다.** 확대 화면에서는
/// 캐릭터가 상대적으로 작아 상판에 가려지는 것처럼 보였지만, 실제 화면(전체 오피스)에서는
/// 0.52 가 캐릭터를 상판 **위로** 올려놓아 다리와 신발이 책상에 얹혔다.
///
/// **캐릭터 배율·바닥 좌표 방식·좌석 원화·앞뒤 순서 중 어느 하나가 바뀌면 이 값도 다시 재야
/// 한다.** 0.12 는 배율 1.0 시절 값이고, #561 에서 배율을 0.85 로 내리자 상판이 덜 가려 다리가
/// 드러나 0.30 으로 올렸다. 방 그림별 바닥 보정(`OfficeFloorCalibration`)으로 줄 간격이 좁아진
/// 뒤에는 상반신만 있는 착석 원화(`sit`)의 아랫부분이 상판 아래로 비어져 나와(사용자 보고:
/// "캐릭터가 책상을 뚫고 지나간다") 0.60 까지 올려야 했다. **그 0.60 의 전제는 "상판이 하반신을
/// 가려 준다" 였다** — 좌석 원화가 전신으로 바뀌면서 그 전제가 사라졌다.
///
/// 좌석 그림이 **뒷모습 전신**(`cozyDeskSeatPose`)으로 바뀌고, 그 몸을 책상 앞판보다 **앞에**
/// 그리도록(`OfficeScene.placeAtWorkstation` 의 `depth`) 고친 뒤의 값이다. 두 전제가 함께
/// 바뀌므로 한쪽만 보고 이 값을 옮기면 안 된다.
///
/// **재는 순서가 결과를 바꾼다.** 앞뒤 순서를 고치기 **전**에는 0.00·0.15·0.30·0.45 중 0.30 이
/// 가장 나았다(1200×800·14시) — 그때는 상판이 하반신을 가려 주고 있어서 몸을 책상 쪽으로
/// 끌어올려야 손이 상판에 닿았다. 순서를 고쳐 몸이 앞판 앞에 서게 된 **뒤**에 0.00·0.15·0.30 을
/// 다시 구우니 답이 내려왔다(0.15) — 가림이 없어진 만큼 덜 올려야 한다.
///
/// **세 번째 측정(2026-09-22, 2000×1250·14시, 0.15·0.05·−0.05·−0.15).** 앞의 두 번은 창이
/// 1200×800 이었고, 실제 앱의 큰 화면에서는 0.15 가 여전히 높아 몸통이 상판 위로 올라와 있었다
/// (사용자 보고: "전반적으로 아래로 내려가면 자연스럽진 않을까요").
///
/// **이 축은 한 값으로 두 불만을 함께 없앨 수 없다.** 3/4 부감이라 내리는 것이 곧 앞으로
/// 나오는 것이어서, 상판에 걸치는 몫을 줄이면 의자가 책상 앞으로 더 나온다. −0.15 는 모니터가
/// 온전히 보이는 대신 의자가 책상에서 떨어져 "책상 앞에 따로 놓인 의자" 가 됐다. −0.05 에서
/// 어깨가 상판 아래로 내려가 모니터가 드러나면서도 의자가 책상에 붙어 있다.
///
/// 대표(`OfficeScene.renderPresident`)도 같은 상수를 쓰지만 **깊이 배율을 곱하지 않는다** —
/// 담당자 쪽만 보고 옮기면 대표 혼자 어긋난다. 위 네 값은 두 자리를 함께 구워 비교했다.
public let officeWorkstationSeatVisualOffsetTiles: Double = -0.05
...
/// 소파 착석(`sitting`)이 가구 쪽으로 당겨지는 양. 가구 방향이 고정되지 않아, 책상용 수직
/// 오프셋 대신 바라보는 쪽으로 당긴다.
///
/// 0.22칸은 몸이 가구에 닿지 않아 바닥에 쭈그려 앉은 것처럼 보였고, 0.48칸은 높이 약 1.35칸인
/// 캐릭터가 1×1 소파를 통째로 덮었다. 그 사이 0.30칸을 오래 썼는데, **그 두 실패는 정면
/// 상반신 원화(`sit`) 시절에 잰 것이다.** 지금 소파에 놓이는 그림은 무릎을 굽히고 발이 바닥에
/// 닿은 전신(`sitting`)이라 같은 값에서 소파를 덮지 않고, 대신 좌판에 닿지도 못해 옆 바닥에
/// 앉은 그림이 됐다(사용자 보고: "여전히 소파가 아닌 공중에 앉아있습니다").
///
/// 사람은 가구 **앞 칸**에 서므로(`officeInteractionNeighbors`) 좌판에 올라앉으려면 한 칸에
/// 가까운 거리를 들어가야 한다. 3/4칸에서 엉덩이가 좌판에 얹히고 등받이가 몸 뒤로 남는다.
/// 책상 좌석의 `officeSeatedSpriteDrop`은 별도 수직 오프셋으로 유지한다.
///
/// **값이 이진수로 딱 떨어지는 것(3/4)을 골랐다.** 0.78 로도 화면은 같았지만 `tileSize × 값`
/// 이 0.0000000000000018 만큼 어긋나 기대값을 손으로 적은 단언이 깨진다. 그 오차를 허용하는
/// 비교로 눌러 두면 이 상수의 회귀를 잡는 힘도 같이 무뎌지므로, 값 쪽을 맞췄다.
public let officeLoungeSpriteShift: Double = 0.75

/// 테이블 앞 착석(`sittingAtTable`)이 당겨지는 양 — **소파와 갈라 둔다.**
///
/// 둘의 요구가 정반대라 한 값으로 맞출 수 없다. 소파는 **좌판 위에 올라앉아야** 하므로 가구
/// 칸 안까지 들어가야 하고, 테이블은 **앞에 의자를 놓고 앉는** 그림이라 앞 칸에 머물러야 한다.
/// 같은 값을 쓰던 동안 소파는 덜 들어가 옆 바닥에 앉고 테이블은 더 들어가 의자째 상판을
/// 관통했다 — 한쪽을 고치면 반드시 다른 쪽이 나빠지는 배치였다.
///
/// 0 이 아닌 이유는 그림이 붙어 보여야 하기 때문이다. 완전히 떼면 테이블과 사람 사이에 빈
/// 바닥이 한 줄 생겨 "그 테이블을 쓰는 중" 으로 안 읽힌다. 1/8 칸은 위와 같은 이유로 고른
/// 이진수로 떨어지는 값이다.
public let officeTableSeatSpriteShift: Double = 0.125
...
public enum FloorTile: String, Codable, Sendable, CaseIterable {
    ...
    public var muteStrength: Double {
        ...
        case .woodB:
            return 0.04
        // 통로는 방 섬과 구분되는 따뜻한 중성색이다. 새 코지 디자인에서는 방보다 어둡게
        // 두어 섬 사이의 경계를 만든다.
        //
        ...
        //
        // 이 값은 화면 실측 게이트(`--color-check`)가 방 바닥보다 충분히 어두운지 확인한다.
        //
        // 다만 위로 끝까지 밀면 안 된다. 한때 셔츠(실측 185.7)와 밝기가 겹쳐 같은 문제가
        // 반대편에서 되돌아온 적이 있다 — 복도는 **방과 사람 사이**에 있어야 한다.
        //
        // 최종 순서와 명암은 `--color-check`가 렌더 픽셀을 직접 측정해 검증한다.
        //
        // **다른 타일의 이 값과 비교해서 밝기 순서를 추론하면 안 된다.** 원본 텍스처 밝기가
        // 종류마다 달라 누르는 양의 순서가 결과 밝기의 순서와 다르다 — 통로(0.06)는 woodB(0.04)
        // 보다 더 눌리는데 텍스처가 밝아(236.9 대 213.3) 화면에서는 통로가 밝다.
        // 밝기 판정은 `swift run IdaeriConsole --color-check` 픽셀 실측으로만 한다.
        case .corridor:
            return 0.06
...
public func departmentFurniture(_ department: Department) -> [FurnitureKind] {
    ...
    case .evaluation:
        ...
        //
        // **다섯뿐이라 아래 줄과 오른쪽 절반이 빈 바닥이었다.** 후보를 넓혀(위 함수 참조)
        // 아홉으로 늘린다. 아래 줄(1,0)~(3,0)은 벽에 등 댄 **자료 라인**으로 읽히게
        // 책장·캐비닛·작은 화분을 붙여 세운다.
        //
        // **복합기는 아래 줄이 아니라 위 줄 (6,4) 다.** 아래 줄에 두면 2열 배치에서 아랫방
        // 사람을 끌어온다 — 근거는 자리 후보 목록의 (6,4) 주석에 있다.
        //
        // **복합기는 장식이 아니다.** 이 방 업무 회고(`WORK_REVIEWER`)와 저녁 회고
        // (`EVENING_RETRO`)의 `officeWorkAffinity` 가 1순위로 복합기를 가리키는데 방에 없어서,
        // 둘 다 방을 나가 품질 방까지 걸어가고 있었다(#499 의 `strollTargetOutOfZone` 명단).
        // 검토한 것을 뽑아 내보내는 방이라 성격에도 맞는다.
        //
        // **다만 3열에서만 둘 다 해결된다.** 2열은 품질 방이 바로 위층이라 `EVENING_RETRO`
        ...
        return [
            ...
            // 아래 줄 자료 라인과 위 줄 빈 열 — 후보를 넓히며 더한 넷.
            // 복합기가 맨 뒤인 것은 자리 때문이다(위 함수의 (6,4) 주석 참조).
            .bookshelf, .filingCabinet, .plantSmall, .printer,
        ]
    case .treasury:
    ...
    case .content:
        ...
        // 후보만 아래 줄·오른쪽 끝까지 넓히니 세트를 아홉으로 늘릴 수 있었다.
        //
        // **책장·복합기·캐비닛 셋은 장식이 아니다.** 이 방 일곱 명의 `officeWorkAffinity` 가
        // 저마다 그 셋 중 **하나 이상**을 가리키는데(원고를 꽂고 · 글을 뽑고 · 지원 서류를
        // 넣는 일이다), 방 안에 하나도 없어서 **일곱 명 모두 방을 나가 품질 방까지 걸어가고
        // 있었다** — 운영 감독이 자기 방에 볼 것이 없어 성장 방 모니터까지 갔던 것과 같다.
        return [
...
///
/// **문 안쪽 한 칸(상대 y = 3 의 x = 1 · 9)도 뺀다.** 복도로 나가는 문은 좌우 벽의 y = 3 에
/// 나는데(`corridorDoorRows`), 문 자신은 통행 가능해도 그 **안쪽 칸**을 가구가 막으면 아무도
/// 그 문을 못 쓴다. 기획 방은 복도 문이 오른쪽 하나뿐인데 (9,3) 에 화분이 서 있어, 복도를
/// 낸 뒤에도 사람이 천장 문으로만 드나들었다 — 천장 문으로 돌아가면 도달은 되므로 도달성
/// 테스트가 통과했고, 화면에서는 "문 앞에 놓인 화분" 으로만 보여 눈에 띄지 않았다.
/// 품질 방도 오른쪽 문이 같은 이유로 막혀 있었다(왼쪽 문이 살아 있어 증상이 더 옅었다).
/// 지금은 「문 안쪽 칸이 통행 가능」 단언이 이 회귀를 잡는다.
public func departmentFurnitureSpots(_ department: Department) -> [TilePoint] {
    ...
    case .planning:
        ...
        // 오른쪽 벽 후보가 (9,3) 이었을 때는 화분이 **복도 문 바로 앞**에 섰다 — 이 방은
        // 왼쪽이 건물 바깥벽이라 복도 문이 오른쪽 하나뿐이고, 그 하나가 막혀 있었다.
        // 한 칸 위(9,4)로 옮긴다. 그 칸은 자리표가 끝까지 차도 책상이 오지 않는다.
        // 뒤 두 자리는 빈 아래쪽을 메우는 몫이다. **맨 아래 줄에만 더한다** — 아래 행 좌석
        // (책상 (1,1)·(7,1) 의 윗칸)에 사람이 앉으면 그 이름표가 y=3 언저리에 뜨므로,
        // 거기 가구를 세우면 인원이 늘었을 때 이름이 가구에 묻힌다.
        return spots([(4, 2), (9, 5), (9, 4), (9, 1), (1, 0), (3, 0), (6, 0), (3, 3), (3, 2), (9, 2)])
    case .quality:
        ...
        //
        // 두 번째 후보는 (9,3) 이었는데, 거기 선 책장이 **오른쪽 복도 문 안쪽**을 막고 있었다
        // (기획 방과 같은 문제 · 이 함수 주석 참조). 가운데 띠의 (5,3) 으로 옮긴다.
        //
        // **(7,3) 이 아니다.** 이 방 아래 줄 좌석은 x=1·4·7 이라, 정원(6명)이 차면 (7,1)
        // 책상에 앉은 사람의 이름표가 (7,3) 높이로 올라온다 — 150cm 책장과 그 이름표·
        // 말풍선이 겹친다. 3명 표본에서는 그 좌석이 비어 있어 렌더로도 안 보인다.
        // 좌석 열이 아닌 x=5 로 옮겼고, 「후보가 좌석 열의 y=3 을 침범하지 않는다」 단언이
        // 이 실수를 막는다.
        //
        // **(8,0) 은 (9,0) 으로 옮겼다 — 문 열이다.** 아래 구역(콘텐츠) 천장문 바로 위
        // 칸이라, 거기 선 책장이 두 방을 잇는 유일한 세로 통로를 막고 있었다. 이 방 안에서는
        // 아무 증상이 없어서(막힌 것은 아랫방에서 올라오는 길이다) 여섯 방을 훑어도 안
        // 보였다 — 문 열 금지 규칙이 이미 있었는데 이 목록만 그 규칙 밖에 있었다.
        //
        // **빼지 않고 옮긴 것은 후보 개수가 곧 여유이기 때문이다.** 하나를 지우면 커서가 한
        // 칸씩 당겨져, 인원이 늘었을 때 뒤쪽 세로 쌍 (6,3)·(6,2) 까지 닿는다 — 그러면 이 방에
        // 세로 쌍이 둘이 되어 그 사이 예비 격자 좌석이 갇힌다(도달성 증원 표본이 잡았다).
        return spots([
            (9, 5), (5, 3), (9, 1), (2, 0), (6, 0),
            (9, 4), (9, 2), (4, 0), (9, 0),
            (2, 3), (2, 2), (6, 3), (6, 2)])
    case .evaluation:
        ...
        //
        // **막고 있던 것은 방 크기가 아니라 후보 목록이었다**(콘텐츠 방 #491 과 같은 진단).
        // 일곱 자리 중 (1,1) 은 늘 책상이라 죽은 자리였고, 실제로 쓰인 것은 다섯뿐이라
        // 아래 줄과 오른쪽 절반이 통째로 빈 바닥이었다. 자리표는 그대로 두고 후보만 넓힌다.
        //
        // **아래 줄(y=0)은 띄워서 쓴다 — 붙여 놓던 것을 되돌렸다.** 붙여 놓으니 왼쪽 끝
        // (1,0) 책장이 네 이웃을 전부 잃어 목적지 목록에서 빠졌다: 좌는 바깥벽, 아래는 바닥벽,
        // 우는 옆 집기, 위 (1,1) 은 늘 책상이다(#512 명단이 이름으로 집어냈다).
        //
        // **「붙여서 놓는다」는 콘텐츠 방 조건에서 나온 규칙이다.** 그 방은 y=1 이 책상·칸막이로
        // 촘촘해 y=0 으로 내려오는 길이 문 열 하나뿐이라, 띄우면 사이 칸이 주머니가 되고 그
        // 주머니를 유일한 앞자리로 고른 가구가 빠진다. 이 방은 사정이 반대다 — y=1 의 짝수 열
        // (x=2·4·6·8)이 다 비어 있어 **가구마다 자기 위 칸을 앞자리로 쓸 수 있고**, 사이에 생기는
        // 빈칸(1,0)·(3,0)·(5,0) 은 아무도 목적지로 삼지 않으므로 갇혀도 무해하다.
        //
        // **한 칸씩 고르게 띄우는 것도 안 된다.** (2,0)·(4,0)·(6,0) 으로 두면 (5,0) 이 사방으로
        // 막혀, 그 칸을 앞자리로 쓰던 (5,1) 캐비닛이 (4,1) 로 밀리고 그 (4,1) 을 노리던 (4,0) 이
        // 대신 빠진다. 그래서 (5,1) 캐비닛의 앞칸 (5,0) 을 살려 두는 (2,0)·(6,0)·(9,0) 을 쓴다.
        //
        // **(8,0) 은 넣지 않는다 — 문 열이다.** 아래 구역 천장문 바로 위 칸이라, 거기 가구를
        // 세우면 아랫방에서 올라오는 유일한 세로 통로가 막힌다(품질 방이 그랬다).
        //
        // **(1,3)·(9,3) 도 넣지 않는다 — 복도 문 안쪽이고, 배치마다 어느 쪽인지 갈린다.**
        // 3열에서 이 방은 맨 오른쪽 열이라 복도 문이 왼쪽(rel 0,3)에만 나고, 2열에서는 왼쪽
        // 열이라 오른쪽(rel 10,3)에 난다. 한쪽만 비우면 다른 배치에서 그 문이 막힌다.
        //
        // **y=3 의 x=1·3·7 은 이름표 열이다** — 이 방 아래 줄 좌석이 x=1·3·7 이라 정원(6명)이
        // 차면 그 높이로 이름표가 올라온다. 그래서 가로 띠에서 쓸 수 있는 것은 x=4·6 뿐이고,
        // 둘 다 쓰면 세로 쌍이 둘이 되므로 (4,3)·(4,2) 한 쌍만 남기고 (6,3) 은 뒤로 뺐다.
        //
        // **복합기 자리는 아래 줄이 아니라 위 줄(6,4)이다.** 아래 줄에 두었더니 2열 배치에서
        // 콘텐츠 방 `BLOG`·`BLOG_PUBLISH` 가 자기 방 복합기 대신 이쪽으로 왔다 — 2열은 평가
        // 방 아래 줄과 콘텐츠 방 위 줄이 벽 하나 사이라 (2,0) 이 그 둘에게 더 가깝다.
        // 방 안쪽 위로 올리면 거리가 뒤집혀 양쪽 다 자기 방 것을 쓴다.
        return spots([
            (3, 4), (7, 4), (5, 1), (9, 1), (4, 3),
            (2, 0), (6, 0), (9, 0), (6, 4),
            (2, 4), (6, 3), (4, 2), (3, 0),
    ...
    case .treasury:
        ...
        //
        // **커피테이블은 소파의 대각선 아래(3,3)다.** 한때는 같은 줄 옆 칸(3,4)이었다 —
        // 깔개 한 장이 소파·테이블·앉는 자리를 모두 담고, 소파 옆에 사이드 테이블이 놓인
        // 응접 세트로 읽힌다는 이유였다.
        //
        // **그 자리는 소파와 겹친다.** 2인 소파는 그림이 두 칸 가까이(실효 1.78칸) 퍼지므로
        // 옆 칸에는 무엇을 놓아도 0.5칸 넘게 물린다. 테이블만 있던 시절에는 물린 자리가
        // 상판이라 "소파 앞에 놓인 테이블" 로 넘어갔는데, 테이블이 안락의자 두 짝을 갖게
        // 되면서 그 의자가 소파에 파묻혔다(사용자 보고, 실앱 화면).
        //
        // 대각선으로 내리면 줄이 달라져 화면에서 세로로 벌어지고 겹침이 사라진다. 예전에
        // 이 자리를 물렸던 이유("둘이 아무 관계가 없다")는 **민 테이블이었기 때문**이다 —
        // 지금은 의자가 함께 그려져 그 자체로 응접 세트라 소파와 나란할 필요가 없다.
        // 소파 정면(4,3)은 그대로 앉는 자리로 비워 둔다.
        //
        // **깔개는 자리를 고르는 기준이 아니다.** 옛 판단이 "테이블은 깔개 안에 있어야
        // 한다" 를 제약으로 들고 있었는데, 깔개는 `officeCozyDrawnFurnitureKinds` 에 없어
        // 완성형 방 셸에서는 **아예 그려지지 않는다** — 화면에 없는 것에 배치를 맞추고
        // 있었다. 놓는 것 자체는 남겨 둔다(도트 배경으로 되돌릴 때 쓰인다).
        //
        // **서가는 (9,4) 가 아니라 (7,4) 다 — 벽 자리 앞칸을 비워 두려고 옮겼다.** 이 방
        // 오른쪽 벽 rel(10,4) 에 걸린 지표 모니터는 방 안쪽 이웃이 (9,4) 인데, 거기 서가가
        // 서 있으면 반대쪽 **복도 칸**을 앞자리로 잡는다. 그 복도 칸은 마주 보는 콘텐츠 방
        // 벽 판과 **같은 칸**이라(방 사이는 벽·복도·벽 세 칸), 먼저 등록되는 이 방이 가져가면
        // 3열 배치에서 콘텐츠 벽 판이 목적지 목록에서 조용히 빠졌다.
        //
        // (9,4) 를 비우니 모니터가 방 안에 서고 복도 칸이 콘텐츠 쪽으로 돌아간다 — 이 방
        // 사람 셋이 자기 방에서 화면을 보게 되는 것까지 함께 나아진다. 원장은 이 결함을
        // 「바닥 가구를 옮겨야 하고 그건 자리표를 다시 세는 별개의 작업」으로 미뤄 뒀는데,
        // 이 방은 좌석이 넷뿐이라 서가를 한 칸 옮기는 것으로 끝났다.
        return spots([(4, 4), (3, 3), (8, 1), (7, 4), (9, 1), (9, 2), (4, 1), (1, 0), (3, 2), (5, 3), (5, 2)])
    case .content:
        // 어긋난 자리 사이를 파티션으로 나누고, 아래 벽에 자료·발행 라인을 붙인다.
        //
        // **후보를 층으로 나눠 읽는다.** y=1 은 자유석 사이 칸막이와 소파, y=2~3 은 그
        // 뒤를 받치는 화분, y=0 은 벽에 등 댄 자료·발행 라인이다. 예전 목록은 산 자리 다섯
        // 중 넷이 y=1 한 줄이었고 세트가 넷뿐이라 **실제로 놓인 가구가 전부 그 줄에** 섰다.
        // 그래서 아래 줄과 가운데 띠 두 줄이 통째로 빈 바닥이었다.
        //
        // 죽은 후보 둘을 뺐다 — (3,4) 는 위 줄 책상이 늘 앉는 칸이고 (9,5) 는 그 책상의
        // 좌석이라, 일곱 후보 중 다섯만 실제로 쓰였다. 오른쪽 문 앞 (9,3) 도 뺐다
        // (이 함수 주석의 「문 안쪽 한 칸」 참조).
        //
        // **아래 줄은 붙여서 놓는다.** 자리표가 다 차면 y=1 이 책상 넷·칸막이 둘·소파로
        // 메워져 y=0 으로 내려오는 길이 문 열(x=8) 하나만 남는다. 그 상태에서 자료 라인을
        // 한 칸 걸러 놓으면 그 사이 칸이 사방으로 막힌 **주머니**가 되고, 주머니는 걸어서 못
        // 가는 칸이라 그 앞에 선다고 고른 가구가 목적지에서 조용히 빠진다(운영실 설비 세 종이
        // 그렇게 빠진 적이 있다 — `officeStrollSpots`). 붙여 놓으면 가운데 책장(2,0)만 설
        // 자리를 잃는데, 세 칸짜리 자료 벽의 가운데라 사람이 설 이유도 없는 자리다.
        //
        // **소파는 y=0 에 못 놓는다.** 앉는 가구의 앞자리는 정면 한 칸뿐이고
        // (`officeInteractionNeighbors`) 정면은 (x, y-1) 이라, 아래 줄에 놓으면 정면이 바닥
        // 벽이 되어 아무도 앉지 못한다. 라운지를 아래 줄에 만들려다 여기서 막혀 (7,1) 로
        // 올렸다 — 정면 (7,0) 이 비어 있는 자리다.
        //
        // 뒤 두 자리는 예비다. 지금은 둘 다 남지만, 인원이 자리표를 넘겨 예비 격자
        // (x = 1·3·5·7, y = 1)까지 쓰면 앞 후보가 책상에 막히므로 여기로 밀린다
        // (가구는 책상보다 나중에 놓여 알아서 비켜난다). 예비도 **아래 줄 좌석 열
        // (x = 2·4·6·9)의 y=3 은 피한다** — 그 높이가 이름표 자리다.
        return spots([
            (3, 1), (5, 1), (7, 1), (7, 2), (5, 3),
            (1, 0), (2, 0), (3, 0), (9, 0),
            (7, 3), (3, 3),
    ...
    case .internalOps:
        // 10명이 x=1~9 를 다 쓰므로 설비는 맨 아래 줄(y=0)과 가로 통로(y=3)로 내려간다.
        //
        // 예전 목록의 뒤 두 자리는 둘 다 못 쓰는 자리였다 — (8,0) 은 위 규칙이 금지한 문 열이고
        // (9,5) 는 (9,4) 책상의 좌석이라 배치 루프가 건너뛴다. 설비가 셋뿐이라 거기까지 커서가
        // 가지 않아 드러나지 않았을 뿐이다. 실제로 쓸 수 있는 양 끝 칸으로 바꿨다.
        //
        // **그 뒤에도 설비 셋이 목적지 목록에서 빠져 있었다**(#512 명단: 복합기·자판기·캐비닛).
        // 이 방은 좌석이 열 개라 앞자리로 쓸 수 있는 칸이 극히 좁고, 그 좁은 칸을 서로 물고
        // 있었다. 세 가지가 겹쳐 있었다.
        //
        // - **아래 줄 왼쪽 두 칸을 붙여 놓았다.** (1,0) 자판기는 좌가 바깥벽·아래가 바닥벽·
        //   우가 복합기·위 (1,1) 이 책상이라 네 이웃이 전부 죽는다.
        // - **홀수 열 y=1 은 전부 책상이라 홀수 열 아래 줄이 주머니가 된다.** (2,0) 복합기의
        //   우 (3,0) 과 뒤 (2,1) 이 그래서 「walkable 이지만 걸어서 못 가는 칸」이었다.
        // - **한 열에 (x,0) 과 (x,2) 를 함께 놓으면 그 사이 (x,1) 이 갇힌다.** 좌우가 책상이라
        //   위아래를 막는 순간 사방이 닫힌다 — 예전 (2,0)+(2,2) 가 정확히 그 모양이었고,
        //   (2,2) 캐비닛은 앞칸이 그 갇힌 칸이라 함께 빠졌다.
        //
        // 그래서 **짝수 열 세 개(2·4·6)에 층을 나눠 하나씩** 둔다. 아래 줄 것은 자기 위 칸
        // (x,1) 에, 가로 통로 것은 자기 위 칸 (x,2) 에 서게 되어 앞자리가 겹치지 않는다.
        // (9,0) 사물함만 앞자리를 옆 (8,0) 에서 받는다.
        //
        // **y=3 의 짝수 열이라야 한다.** 홀수 열은 아래 줄 좌석(x=1·3·5·7·9)의 이름표가
        // 뜨는 높이이고, 동시에 위 줄 책상 바로 아래 칸이다.
        //
        // **자리와 설비 수가 딱 맞다(일곱에 일곱).** 여덟 번째를 요청하면 조용히 빠지는데,
        // 「요청한 가구가 전부 놓였는가」 단언이 이름으로 잡는다(#512).
        return spots([(2, 0), (4, 0), (6, 0), (9, 0), (2, 3), (4, 3), (6, 3)])
    }
}
