...
///
/// 캐릭터를 **책상보다 앞(아래)** 에 두어야 한다. 그리는 순서는 이미 캐릭터가 뒤이므로
/// (`depth(of: seat) - 0.24`), 겹치는 만큼 상판이 하반신을 가려 앉은 자세로 읽힌다.
/// **캐릭터 배율이나 바닥 좌표 방식이 바뀌면 이 값도 다시 재야 한다.** 0.12 는 배율 1.0
/// 시절 값이고, #561 에서 배율을 0.85 로 내리자 상판이 덜 가려 다리가 드러나 0.30 으로 올렸다.
///
/// 0.60 이던 시절의 근거는 **상반신만 있는 착석 원화**(`sit`)였다. 방 그림별 바닥 보정
/// (`OfficeFloorCalibration`)으로 줄 간격이 좁아지자 0.30 에서는 그림의 아랫부분이 상판
/// 아래로 비어져 나와(사용자 보고: "캐릭터가 책상을 뚫고 지나간다") 0.60 까지 올려야 했다.
///
/// 지금 값 0.30 은 좌석 그림이 **뒷모습 전신**(`cozyDeskSeatPose`)으로 바뀐 뒤 다시 잰 것이다.
/// 의자와 다리까지 그려져 있어 예전만큼 올릴 필요가 없다 — 올리면 그 전신이 통째로 상판 위로
/// 떠오른다. 같은 창 크기(1200×800)·같은 시각(14시)으로 0.00·0.15·0.30·0.45 를 굽고 기획 방의
/// 가운데·왼쪽 두 책상에서 비교했다. 0.00 과 0.15 는 다리와 의자가 상판 아래로 크게 나오고,
/// 0.45 는 몸이 상판 위로 올라가 의자가 책상에 얹힌다. 0.30 에서 손이 키보드에 닿고 의자
/// 등받이가 상판 뒤로 자연스럽게 걸린다.
