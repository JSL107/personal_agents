...
public func officeFloorPlan(agents: [ConsoleAgent]) -> OfficeFloorPlan {
    ...
    // 자리 계산은 `officeSessionTiles` 가 방 왼쪽 끝부터 두 칸 간격으로 잡으므로 여기서
    // 어긋나면 사람 수가 조용히 줄어든다.
    //
    // 쓸 수 있는 홀수 칸은 1·3·7·9 넷이다(5 는 대표가 서는 칸). 복도 작업에서 함께 넣었던
    // 커피 테이블은 그래서 뺐다 — 다섯 번째 가구를 놓으려면 짝수 칸을 써야 하고, 그 순간
    // 앉을 자리가 하나 조용히 사라진다.
