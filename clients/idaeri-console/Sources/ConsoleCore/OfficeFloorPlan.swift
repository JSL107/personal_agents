...
///
/// 지금 값 0.15 는 좌석 그림이 **뒷모습 전신**(`cozyDeskSeatPose`)으로 바뀌고, 그 몸을 책상
/// 앞판보다 **앞에** 그리도록(`OfficeScene.placeAtWorkstation` 의 `depth`) 고친 뒤에 잰 것이다.
/// 두 전제가 함께 바뀌므로 한쪽만 보고 이 값을 옮기면 안 된다.
///
/// **재는 순서가 결과를 바꾼다.** 같은 창 크기(1200×800)·같은 시각(14시)으로 두 번 쟀다.
/// 앞뒤 순서를 고치기 **전**에는 0.00·0.15·0.30·0.45 중 0.30 이 가장 나았다 — 그때는 상판이
/// 하반신을 가려 주고 있어서 몸을 책상 쪽으로 끌어올려야 손이 상판에 닿았다. 순서를 고쳐
/// 몸이 앞판 앞에 서게 된 **뒤**에 0.00·0.15·0.30 을 다시 구우니 답이 내려왔다 — 가림이
/// 없어진 만큼 덜 올려야 한다. 0.30 은 몸이 상판 위로 올라가고, 0.00 은 상판 아래로 처진다.
/// 0.15 에서 손이 키보드에 닿고 의자 등받이가 상판 뒤로 자연스럽게 걸린다.
public let officeWorkstationSeatVisualOffsetTiles: Double = 0.15
