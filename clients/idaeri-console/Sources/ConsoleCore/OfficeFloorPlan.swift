...
///
/// 지금 값 0.60 은 **방 그림별 바닥 보정**(`OfficeFloorCalibration`)을 넣은 뒤 다시 잰 것이다.
/// 바닥을 원근에 맞추자 줄 간격이 좁아져 0.30 에서는 상반신만 있는 착석 원화의 아랫부분이
/// 상판 아래로 비어져 나왔다(사용자 보고: "캐릭터가 책상을 뚫고 지나간다"). 같은 창 크기·같은
/// 시각으로 0.30·0.45·0.55·0.60·0.68 을 굽고 기획·콘텐츠 두 방에서 비교했다 — 0.30 은 몸이
/// 책상 아래로 나오고, 0.68 은 사람이 상판 위로 떠오르기 시작한다. 0.60 에서 손이 상판에
/// 닿고 하반신이 가려진다.
