...
목표 5 는 #525 머지로 절반만 닫혔다 — 실행 확인이 남아 목표 7·9 의 전제는 아직 미충족)
...
`docs/` 는 `.gitignore` 대상이라 이 스펙은 `git add -f` 로 저장소에 넣었다 — 원장만 받은
세션이 목표 9 의 크기 예산·트리 `truncated` 처리와 목표 밖 3단계 계약을 읽지 못하면
인수인계가 반쪽이 된다(같은 사고를 #452 에서 이미 한 번 고쳤다).
...
목표 5 는 머지(#525)만 끝났고 **실행 확인이 남아 아직 열려 있다.** 따라서 지금 바로 착수
가능한 것은 **목표 6(코드 0, 조사만)** 과 **목표 5 의 남은 실행 확인** 둘이다.

---

## 목표 5 — #525 머지 (학습 재료 위생 + `review:dry`)

- [ ] **상태**: 진행 중 — 머지는 끝났고(2026-09-09, PR #525 `8e056216`) **실행 확인이 남았다.**
  🔴 체크박스를 채우지 않는 이유: 이 원장은 "착수 전에 `전제` 를 확인" 으로 돌아간다. 여기를
  `[x]` 로 두면 목표 7·9 를 여는 세션이 **결과란의 미검증 경고를 안 읽고** 지나간다 —
  그 경고가 가리키는 사고가 정확히 "머지된 PR 에 진짜 코멘트가 달린다" 다.
- **전제**: 없음
- **왜**: 이 국면의 나머지가 전부 여기 기댄다.
  - `pnpm review:dry`(`scripts/review-pr-dry.ts`) 없이는 **게시 없이 과거 PR 을 재리뷰할 방법이
    없다** = 목표 9·10 의 검증 자체가 불가능하다. (머지로 스크립트는 main 에 들어왔다.
    남은 것은 그것이 정말 게시를 막는지 한 번 굴려 보는 일이다.)
  - 목표 7 이 같은 파일(`harvest-review-signals.usecase.ts`)의 `case 'ACKED'` 를 건드린다.
    #525 가 그 파일에 +83줄을 얹었고 main 에는 #527 이 먼저 들어가 있었다 — 셋이 겹쳤다.
    (머지로 해소됨.)
...
- [ ] **상태**: 미착수
- **전제**: 목표 5 **완료(실행 확인까지)**. `review:dry` 가 게시를 막는 것을 **한 번이라도
  확인한 뒤**에 연다 — 확인 없이 이 목표의 검증을 돌리면 머지된 PR 에 실제 코멘트가 달린다.
  체크박스가 아니라 이 문장이 게이트다.
