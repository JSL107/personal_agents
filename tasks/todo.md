# 콘솔 오피스 — 세션을 "출퇴근하는 사람"으로 (2026-08-05)

## 문제

대표실 앞줄에 세션 캐릭터 열 명이 한 칸 간격으로 도열하고, 머리 위 이름표가 서로 이어 붙어
한 줄짜리 글자 뭉치가 됐다. 대부분 반투명(쉬는 세션)이라 "없던 직원이 갑자기 생긴" 유령
무리로 읽혔다. 데이터는 정확했다 — 잡힌 세션 13개 전부 실제로 살아 있는 프로세스였고,
문제는 전적으로 표현이었다.

## Plan

- [x] 회귀 렌더가 세션을 그리게 한다(`syncSessions` 누락으로 이 결함이 점검을 통째로 빠져나갔다).
- [x] 자리를 두 칸 간격으로 띄우고 대표 칸을 비운다.
- [x] 라벨 헬퍼에 외곽선을 넣어 가구 무늬 위에서도 읽히게 한다.
- [x] 세션을 **앉힌다** — 대표실 안쪽 줄, 앞줄은 통로로 남긴다.
- [x] 대표실 가구를 홀수 칸으로 몰아 자리 5개를 확보한다.
- [x] 출근·퇴근 동선을 붙인다(문에서 걸어와 앉고, 오래 조용하면 걸어 나간다).
- [x] 한 번 앉은 세션은 자리를 지키게 한다(`officeAssignSessionSeats`).
- [x] 세션 이름표를 발치로 내려 "나 (대표)" 라벨과 높이를 가른다.

## Review

- **표현을 바꿨지 데이터를 숨기지 않았다.** 화면에서 빠지는 기준은 "쉬는 중"이 아니라
  **15분간 조용함**(`officeSessionLeaveAfterSeconds`)이다. 백엔드의 `idle` 은 60초짜리라
  그것으로 사람을 지우면 답변을 기다리는 사이 사라졌다 나타나기를 반복한다 — 실측으로
  25초 만에 한 명이 사라지는 것을 확인했다.
- 세션은 이제 `CharacterNode` 를 쓴다. 걷기·앉기·이름표·상태 링·타이핑 몸짓을 에이전트와
  공유하므로 세션 전용 렌더 코드가 사라졌다. 부서가 없는 점만 `shirtOverride` 로 처리한다.
- 검증: 콘솔 테스트 810건 통과. 오프스크린 렌더로 수정 전/후 대조(이름표 10개 판독 불가 →
  5개 전부 판독). **출퇴근 걷기는 정지 렌더로 확인할 수 없어 미검증** — 실앱에서 봐야 한다.
- 범위 밖: 상단 밴드 확장은 실험 후 되돌렸다(타일이 10% 작아지고 밴드 가구가 전부 벽에서
  떨어져 붕 뜬다). 자리 5개를 넘는 세션은 좌상단 요약의 총계가 맡는다.
