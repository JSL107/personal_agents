# 완료 작업 로그 아카이브 (2026-08-24 분리)

`tasks/todo.md` 에서 잘라낸 완료 블록. 「남은 것」·「후속」·「미검증」 문단이 한 줄도 없어
다시 볼 이유가 없는 것만 모았다. 시간순은 원본 순서를 그대로 유지한다.

- 블록 70개
- 원본: `tasks/todo.md` (분리 전 2,447줄)
- 잔여 꼬리가 남은 블록은 `tasks/todo.md` 에 그대로 두었다.
...
- [x] PR #329 봇 리뷰 1건(두 봇 동일 지적)을 반영한다 — `pr` 정규화가 형식을 검증하지 않아 다른 PR 번호로 바뀌던 문제.
...
- 리뷰 반영 — Codex(P2) 와 이대리 봇(MUST_FIX) 이 같은 지점을 지적했다. `toPrNumber` 가 비숫자를 전부 제거해
  형식을 벗어난 값을 **다른 PR** 로 바꿨다. 실측 재현: `"1.5"`→15, `"PR-12-extra"`→12, `"-12"`→12(부호 소실),
  `"984#985"`→984985, `984.5`→9845. 그 번호가 실재하면 남의 머지 시각을 근거에 심고 slug·dedup 키까지 그 PR 을
  가리키는 조용한 오귀속이 된다. 허용 형식을 `/^#?(\d+)$/` + 양의 safe integer 로 좁히고, 벗어난 값은 변환하지
  않고 원값을 유지한다(백필은 키 불일치로 건너뜀, slug 미생성 후 `skippedTitles` 계상).
  `buildProjectSlug` 가드도 `Number.isSafeInteger(pr) && pr > 0` 으로 강화했다 — 숫자 `-12`·`0` 은 기존
  `Number.isInteger` 를 통과해 `…-pr--12` 를 만들고 있었다.
- 리뷰 반영 검증 — 형식 밖 5종과 "권위 PR 과 우연히 겹쳐도 mergedAt 을 덮지 않는다" 를 회귀 테스트로 고정하고
  구현 전 6건 FAIL 을 확인했다. 첫 RED 시도는 `it.each` 튜플 형태 때문에 suite 가 컴파일 실패로 **0건 실행**된
  것을 실패로 오독할 수 있었다 — 실패 사유까지 읽어 바로잡았다.
- 리뷰 반영 후 게이트: lint 0(warning 57), tsc 0, test 0(406 suites/3,460 tests + 5 suites/40 tests), build 0, docs:check 0.
  실데이터 재실측도 오염 slug 0건·프로젝트 26건 유지로 동일하다.
