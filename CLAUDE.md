...
- Node 22.12+, NestJS 11, Prisma 6 (TypeORM 절대 X), Slack Bolt 4, BullMQ. (`package.json` `engines` 로 명시 — 20 은 지원 종료. 하한이 22.0 이 아니라 22.12 인 것은 puppeteer 25 가 그것을 요구해서다). **NestJS 11 은 Express 5 를 쓴다** — 라우트 와일드카드 문법과 `req.query` 파서가 4 와 다르다(이 레포는 둘 다 안 써서 이관 시 영향이 없었다)
- DB: **PostgreSQL @ 5434**, Redis @ 6381 (로컬 docker). 다른 포트 가정 X.
- LLM: `codex` CLI (ChatGPT 구독) + `claude` CLI (Claude Max 구독). 자식 프로세스 spawn 으로만 호출 — 직접 API SDK 사용 X. 인증: 기본은 각 CLI 의 keychain/OAuth (구독). Claude 는 keychain ACL 미등록 환경 (nest start --watch child PID 변동 등) 우회용으로 `CLAUDE_CODE_OAUTH_TOKEN` env 경로 지원 — `.env` 에 `claude setup-token` 발급 OAuth token (`sk-ant-oat01-...`) 두면 자식 env 로 forward, docs precedence priority 5 (OAUTH_TOKEN) > priority 6 (keychain) 라 keychain 시도 자체가 안 일어남 (§6 참조). `ANTHROPIC_API_KEY` 도 backward-compat alias 로 동일하게 인식. primary 는 전 에이전트가 ChatGPT(codex) 이고, **2026-09-09 부터 codex 실패 시 Claude 로 한 번 폴백**한다 (둘 다 실패해야 최종 실패, 쿼터 소진 시 reset 시각 안내). 단 `noFallback: true` 요청과 `outputSchema` 를 건 요청은 폴백을 타지 않는다 — claude CLI 에 형태 강제 인자가 없어서다. 이전 Gemini fallback 은 2026-06-04 제거, Claude 는 2026-07-02 제거 후 09-09 복원.
...
- **claude OAuth subscription token 경로** (keychain ACL 우회): `claude setup-token` 으로 발급한 long-lived OAuth token (`sk-ant-oat01-...`) 을 `.env` 의 `CLAUDE_CODE_OAUTH_TOKEN` (또는 backward-compat `ANTHROPIC_API_KEY` alias) 에 두면 ClaudeCliProvider 가 자식 env 의 `CLAUDE_CODE_OAUTH_TOKEN` 으로 forward. docs precedence priority 5 (OAUTH_TOKEN) > priority 6 (keychain) 라 keychain 시도 자체가 안 일어남 = **macOS Keychain ACL 미등록 환경 (nest start --watch 의 child PID 변동) 의 침묵 exit=1 자연 우회**. `--bare` / `CLAUDE_CODE_SIMPLE=1` 은 안 씀 (docs 의 Bare/SIMPLE 모드는 `ANTHROPIC_API_KEY` 를 API key `sk-ant-api03-` 형식만 받고 OAuth subscription token 은 "Invalid API key" 로 거부 — 2026-06-05 manual test 확정). token 미설정 시 기존 keychain 경로 fallback — **이 fallback 이 안 되면 ACL 부터 의심하지 말 것.** 2026-09-16 까지는 ACL 과 무관하게 아래 `CLAUDE_CONFIG_DIR` 주입 때문에 fallback 자체가 죽어 있었다 (ACL 은 그 다음 후보다).
...
이 레포의 PR 에 `🤖 **이대리 자동 리뷰**` 로 시작하는 인라인 코멘트가 달리면
**답변·리액션 전에 [docs/pr-review-bot-protocol.md](./docs/pr-review-bot-protocol.md) 를 읽고 그대로 따른다.**

봇이 채택/기각을 학습하므로 답변만으로는 부족하다 — 정탐 👍 / 오탐은 반박 답변을 **먼저** 게시한 뒤 👎.
순서를 뒤집으면 스윕이 답변보다 앞질러 돌아 기각 이유가 유실된다. 🔴 정탐에 👎 는 역학습이라 금지.
나머지(식별 표식, API 호출, 되돌리기 `pnpm review:unlearn`, 표식으로 시작 금지)는 위 문서에 있다.
