...
GitHub · Notion · Slack 을 연결해 **회사 롤플레이 역할**(PM · BE · Code Reviewer · CTO · PO · CEO …)과 **개인 업무**(이직 메이트 · 지원 추적 · 휴가 · 블로그)를 함께 수행하는 1인 개발자용 비서 백엔드. 수동 슬래시와 자동 **Autopilot** 플레이북을 병행한다.

자동화 규칙 [AGENTS.md](./AGENTS.md) · 코드 컨벤션 [CODE_RULES.md](./CODE_RULES.md).

## 무엇을 하나

- **슬래시 / 자연어 둘 다** — `/today` 같은 슬래시 18종, 또는 `@이대리 오늘 plan 짜줘` 자연어 멘션·DM 으로 진입
...
- **승인 게이트** — 외부 시스템 쓰기는 항상 Slack ✅/❌ 확인 후 실행
...
- **BE 자율 4종** — `/be schema`(Prisma 스키마 제안) · `/be test`(tree-sitter AST 기반 Jest 생성) · BE-SRE(CI 실패 → stack trace 분석) · BE-FIX(PR 컨벤션 분석) — 뒤 둘은 webhook 자동
...
### 자동화 (Autopilot)
선언적 "워크데이 플레이북" + 얇은 오케스트레이터가 출근(아침 PM 계획)/퇴근(PO_EVAL 회고 + worklog)/주간(Weekly · CEO · Impact · Run-Retro) cron 을 단일 엔진으로 통합. 리스크 티어(읽기·요약 자동 발송 / 외부쓰기는 PreviewGate), 다중 타깃 fan-out, digest 그룹(같은 시각 작업 1메시지), 멱등 + 활동 0이면 skip. 전체 게이트는 `AUTOPILOT_OWNER_SLACK_USER_ID` 한 값.
...
### 연동 · 자동 트리거
- **GitHub Webhook** — issue/PR open → Impact Reporter, PR open → BE-FIX(+조건부 Code Reviewer), CI 실패 → BE-SRE, PR merge → careerLog Notion 적재, issue open → Auto-Label(기존 vocab 안 LLM 분류)
- **Notion** — Daily Plan append · PR careerLog 일별 자식 페이지 누적 · 📌 reaction → to-do 적재
- **운영** — `/search-runs`(성공 run input/output ILIKE 검색) · `/retry-run`(실패 run 재실행) · 인증/cron 실패 owner DM 알림
...
- _(예정)_ 운영 전환 시 `prisma db push` → `prisma migrate dev` 마이그레이션 워크플로우
...
`@이대리 …`(채널) 또는 DM 으로 보내면 Router 가 17 worker 중 1개로 분류·dispatch (결과는 thread 답글 + `agentRunId` 푸터). **BLOG · VACATION · 이직 메이트 · 지원 추적은 자연어 전용**.
설정: Event Subscriptions 에 `app_mention` + `message.im`, Bot scope 에 `app_mentions:read` + `im:history` 추가.

### Webhook (`POST /v1/agent/github`, `POST /v1/agent/trigger`)
...
단일 source-of-truth 는 [app.config.ts](src/config/app.config.ts)(class-validator 강제). 아래는 자주 만지는 키만 — webhook/careerLog/impact 세부 옵션은 해당 파일 주석 참조.
...
> DB 변경: `prisma/schema.prisma` 수정 → `pnpm db:push`(synchronize 방식, Prisma Client 자동 재생성) → 앱 재시작.

## 참고 문서

- [자동화 규칙 (AGENTS.md)](./AGENTS.md) · [코드 규칙 (CODE_RULES.md)](./CODE_RULES.md)
- Slack 봇 최초 설정(슬래시 18개 등록 / App Manifest / Socket Mode) 및 env 세부 옵션은 각 파일 주석 참조
