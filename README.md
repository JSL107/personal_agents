...
| ⚡ **자동 발화** | 출근/퇴근/주간 cron 33슬롯 + GitHub webhook 으로 사용자 입력 없이 proactive 동작 |
...
**상시 스윕** — 발화가 아니라 뒷정리·폴링이라 Slack 으로 오지 않는다.
...
> 원래 한계였던 **생존 편향은 해소됐다** — 재생 유니버스를 시작일 기준으로 잡고 구간 중 폐지되면 청산한다.
